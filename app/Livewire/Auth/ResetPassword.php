<?php

namespace App\Livewire\Auth;

use App\Models\User;
use Illuminate\Auth\Events\PasswordReset;
use Illuminate\Support\Facades\Hash;
use Illuminate\Support\Facades\Password;
use Illuminate\Support\Str;
use Illuminate\Validation\Rules;
use Livewire\Attributes\Layout;
use Livewire\Attributes\Locked;
use Livewire\Component;

#[Layout('components.layouts.auth')]
class ResetPassword extends Component
{
    #[Locked]
    public string $token = '';

    public string $email = '';

    #[Locked]
    public string $broker = 'users';

    public string $password = '';

    public string $password_confirmation = '';

    /**
     * Mount the component.
     */
    public function mount(string $token): void
    {
        $this->token = $token;

        $this->email = request()->string('email')->value();

        // Get broker from URL query parameter, default to 'users' for backward compatibility
        $broker = request()->query('broker');

        // Validate broker - only allow 'users' or 'webusers'
        $this->broker = in_array($broker, ['users', 'webusers'], true) ? $broker : 'users';
    }

    /**
     * Reset the password for the given user.
     */
    public function resetPassword()
    {
        $this->validate([
            'token' => ['required'],
            'email' => ['required', 'string', 'email'],
            'password' => ['required', 'string', 'confirmed', Rules\Password::defaults()],
        ]);

        // Here we will attempt to reset the user's password. If it is successful we
        // will update the password on an actual user model and persist it to the
        // database. Otherwise we will parse the error and return the response.

        // Get the broker from the URL parameter (set when the reset link was sent)
        // Using broker approach works for both 'users' (default) and 'webusers'
        $status = Password::broker($this->broker)->reset(
            $this->only('email', 'password', 'password_confirmation', 'token'),
            function ($user) {
                $user->forceFill([
                    'password' => Hash::make($this->password),
                    'remember_token' => Str::random(60),
                ])->save();

                event(new PasswordReset($user));
            }
        );

        // If the password was successfully reset, we will redirect the user back to
        // the application's home authenticated view. If there is an error we can
        // redirect them back to where they came from with their error message.
        if ($status != Password::PasswordReset) {
            session()->flash('error', __('messages.login.invalid_email_error'));

            return;
        }

        session()->flash('success', __('messages.login.reset_password_success'));

        return $this->redirect('/', navigate: true);
    }

    public function render()
    {
        return view('livewire.auth.reset-password')->title(__('messages.meta_titles.reset_passowrd'));
    }
}
