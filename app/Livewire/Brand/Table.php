...
final class Table extends PowerGridComponent
{
    ...
    public function exportData()
    {
        try {
            ...
            $exportService = ExportServiceFactory::create('brand');
            ...
            if ($totalRecords == 0) {
                $this->dispatch('alert', type: 'error', message: 'No records found to export');
            ...
            $exportParams = [
                'exportType' => 'brand', // Export type identifier
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('add-brand')) {
            $headerArray[] = Button::add('add-brand')
                ->slot('    <a href="/brand/create" title="Add New Brand" data-testid="add_new" class="flex items-center justify-center cursor-pointer" wire:navigate>
        ...
        if (Gate::allows('export-brand')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a wire:click.prevent="exportData" href="#" title="Export Brand" data-testid="export_button" data-export-type="brand"  class="flex items-center justify-center"  wire:target="exportData" onclick="handleExportClick(event)" x-bind:class="$wire.isExporting ? \'opacity-50 cursor-not-allowed\' : \'cursor-pointer\'" x-bind:disabled="$wire.isExporting" >
    ...
    public function datasource(): Builder
    {
        ...
        return Brand::query()
            ...
            ->select([
                'brands.id', 'brands.name', 'brands.remark', 'brands.bob',
                DB::raw(
                    '(CASE
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.brand.listing.id'), 'id')
                ->sortable()
                ->searchable(),
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('name', 'brands.name')
                ->operators(['contains'])
                ->placeholder(__('messages.brand.filter.name_placeholder')),
            Filter::inputText('remark', 'brands.remark')
                ->operators(['contains'])
                ->placeholder(__('messages.brand.filter.remark_placeholder')),
