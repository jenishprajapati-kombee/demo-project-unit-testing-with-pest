...
class ImportProductJob implements ShouldQueue
{
    ...
    {
        try {
            // Fetch pending import logs for products
            $importLogs = ImportLog::where('model_name', config('constants.import_csv_log.models.product'))
            ...
            if (! $importLogs->isEmpty()) {
                foreach ($importLogs as $importLog) {
                    // Log initiation of product data import processing
                    Log::info('Initiate processing of product data import: ' . $importLog->file_name);
                    ...
                    // Initialize productImport model
                    $model = new ProductImport($importLog->user_id);
                    ...
                    $redirectLink = url('product-imports?filename=' . $importLog->file_name);

                    // Set email subject for import notification
                    $subject = config('constants.import_csv_log.subject.product');

                    // Initiate common import process for products
                    Product::commonImport($model, $path, $importLog->model_name, $importLog->file_name, $redirectLink, $subject, $importLog);
            ...
            Log::error('Import product Exception Occurred: ' . $e->getMessage() . PHP_EOL . $e->getTraceAsString());
